...
void registerRepositoryState(controller2* controller_pointer, GenericElement new_history) {
...
int compareBarricadesByVisibility2(barricade* object_1, barricade* object_2) {
	return strcmp(object_1->visibility_in_area, object_2->visibility_in_area);
}


void sortAscending2(barricade** list_of_barricades, int list_length, int(*key)(barricade*, barricade*)) {
...
void sortDescending2(barricade** list_of_barricades, int list_length, int(*key)(barricade*, barricade*)) {
	barricade* auxiliary_pointer = NULL;
	int i, j;
	for (i = 0; i < list_length; i++) {
		for (j = 0; j < list_length; j++) {
...
controller2* createController2(repository* repository_pointer) {
	controller2* new_controller = (controller2*)malloc(sizeof(controller2));
...
int redo2(controller2* controller_pointer) {
	int i;
	if (controller_pointer->repository_index >= controller_pointer->hystory_of_repository_size) {
		return 0;
	}
...
void listSturdinessAscending2(controller2* controller_pointer, char* test_barricade_sturdiness, char* string_to_print) {
	...
	sortAscending(list_of_filtered_barricades, length, compareBarricadesByVisibility2);
...
void listSturdinessDescending2(controller2* controller_pointer, char* test_barricade_sturdiness, char* string_to_print) {
	...
	sortDescending(list_of_filtered_barricades, length, compareBarricadesByVisibility2);
