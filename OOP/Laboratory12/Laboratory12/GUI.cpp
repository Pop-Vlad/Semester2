...
void GUI::connectSignals() {
	QObject::connect(this->file_location, &QPushButton::clicked, this, &GUI::changeFile);
	QObject::connect(this->mylist_location, &QPushButton::clicked, this, &GUI::changeMylist);
	...
	QObject::connect(this->save_button, &QPushButton::clicked, this, &GUI::saveFragment);
	QObject::connect(this->next_button, &QPushButton::clicked, this, &GUI::nextFragment);
	QObject::connect(this->search_button, &QPushButton::clicked, this, &GUI::search);
}
...
void GUI::populateMylist() {
	try {
		QFont font{ "Verdana", 15 };
		this->mylist->clear();
		for (auto current : this->controller->showUserList()) {
			QString current_item_string = QString::fromStdString(current.toString());
			QListWidgetItem* current_item = new QListWidgetItem{ current_item_string };
			current_item->setFont(font);
			this->mylist->addItem(current_item);
		}
	}
	catch (GenericException& exception) {
		QMessageBox messageBox;
...
void GUI::nextFragment() {
	try {
		...
		this->current_fragment->setText(QString::fromStdString(current_fragment.toString()));
	}
	catch (GenericException& exception) {
		QMessageBox messageBox;
		messageBox.critical(0, "Error", exception.what());
...
void GUI::saveFragment() {
	try {
		std::string current_fragment_string = this->current_fragment->text().toStdString();
...
void GUI::search() {
	try {
		std::string size_search = this->size_search_edit->text().toStdString();
		std::string microfragments_search = this->microfragments_search_edit->text().toStdString();
		std::vector<std::string> arguments;
