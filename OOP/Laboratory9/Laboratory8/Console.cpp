...
void Console::run() {
	...
	while (true) {
		try {
			...
			else if (arguments.front() == "mylistLocation") {
				std::string file_path{};
				for (int i = 1; i < arguments.size(); i++) {
					file_path += arguments[i];
				}
				this->controller->changeMyList(file_path);
				std::cout << "MyList location set\n\n";
			}
			else {
...
void Console::printMenu() const {
	...
	std::cout << "mylistLocation <new location> - change the file in which the user data is stored\n";
	std::cout << "For mode A:\n";
